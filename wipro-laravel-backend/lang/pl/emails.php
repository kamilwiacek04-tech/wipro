<?php

return [

    'offer_sent' => [
        'subject'         => 'Oferta handlowa — :number',
        'title'           => 'Wipro — Oferta handlowa',
        'greeting'        => 'Dzień dobry :name,',
        'intro'           => 'Przygotowaliśmy dla Państwa ofertę handlową. Poniżej znajdą Państwo szczegółowe zestawienie.',
        'offer_number'    => 'Numer oferty:',
        'issue_date'      => 'Data wystawienia:',
        'valid_until'     => 'Ważna do:',
        'col_description' => 'Opis',
        'col_qty'         => 'Ilość',
        'col_unit_price'  => 'Cena jedn. netto',
        'col_total'       => 'Wartość netto',
        'net_total'       => 'Suma netto:',
        'vat'             => 'VAT :rate%:',
        'gross_total'     => 'Suma brutto:',
        'vat_disclaimer'  => 'Stawka VAT ma charakter poglądowy, ostatecznie decydujące są krajowe przepisy podatkowe.',
        'notes_label'     => 'Uwagi:',
        'contact'         => 'W razie pytań skontaktuj się z nami:',
    ],

    'quote_submitted' => [
        'subject'      => 'Zapytanie ofertowe zostało przyjęte — :number',
        'title'        => 'Wipro — Potwierdzenie zapytania',
        'greeting'     => 'Dzień dobry :name,',
        'intro'        => 'Dziękujemy za przesłanie zapytania ofertowego. Poniżej znajdziesz jego szczegóły.',
        'number'       => 'Numer zapytania:',
        'date'         => 'Data złożenia:',
        'status'       => 'Status:',
        'status_value' => 'Nowe',
        'next_steps'   => 'Nasi doradcy przeanalizują Twoje zapytanie i skontaktują się z Tobą, aby omówić szczegóły i przygotować ofertę.',
        'contact'      => 'W razie pytań skontaktuj się z nami:',
    ],

];
